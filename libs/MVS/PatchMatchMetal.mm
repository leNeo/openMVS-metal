...
void PatchMatch::EstimateDepthMap(DepthData& depthData)
{
	...
	// reentrant: per-call config is local (EstimateDepthMap may run on several
	// worker threads sharing one instance); only Init() writes the params member.
	const bool geom = params.bGeomConsistency;
	const int nEstimationIters = params.nEstimationIters;
	...
	const int nNumViews = (int)numImages - 1;
	const int nInitTopK = MINF(params.nInitTopK, nNumViews);
	const float fDepthMin = depthData.dMin;
	const float fDepthMax = depthData.dMax;
	const int maxPixelViews(MINF(nNumViews, 4));
	...
	for (unsigned scaleNumber = totalScaleNumber + 1; scaleNumber-- > 0; ) {
		...
		float fThresholdKeepCost = OPTDENSE::fNCCThresholdKeep;
		if (totalScaleNumber) {
			if (scaleNumber > 0 && scaleNumber != totalScaleNumber)
				fThresholdKeepCost = 0.f;
			else if (scaleNumber == totalScaleNumber || (!geom && OPTDENSE::nEstimationGeometricIters))
				fThresholdKeepCost = OPTDENSE::fNCCThresholdKeep * 1.2f;
		...
		MtlParams mp{ fDepthMin, fDepthMax, fThresholdKeepCost,
			nNumViews, nEstimationIters, nInitTopK,
			bLowResProcessed ? 1 : 0, W, Hh };
		...
		NSMutableArray<id<MTLTexture>>* depthTexs = [NSMutableArray arrayWithCapacity:nNumViews];
		...
		auto bindCommon = [&](id<MTLComputeCommandEncoder> enc) {
			...
			for (int v = 0; v < nNumViews; ++v) [enc setTexture:depthTexs[v] atIndex:METAL_MAX_VIEWS+v];
		...
		for (int iter = 0; iter < nEstimationIters; ++iter) {
